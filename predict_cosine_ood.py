...
OOD_STATS  = "실험/cosine_ood_stats.pt"
MAX_LEN    = 96
...
detector = CosineOOD.load(OOD_STATS)

lines = []
correct = 0

header = (f"{'#':>2}  {'정답':^3}  {'예측':^4}  {'신뢰도':>6}  "
          f"{'거리':>6}  {'nearest':^7}  {'OOD':^5}  {'정답?':^4}  메시지")
lines.append(header)
lines.append("─" * 120)
...
for i, (msg, true_label) in enumerate(messages, 1):
    ...
    dist, nearest = detector.score(cls_emb)
    ood = dist > detector._threshold_for(nearest)
    ...
    nearest_str = f"L{nearest}({detector._threshold_for(nearest):.3f})"
    short = msg[:45] + ("…" if len(msg) > 45 else "")
    lines.append(
        f"{i:>2}  L{true_label:^3}  {LABEL_NAMES[pred]:^4}  {probs[pred]*100:5.1f}%  "
        f"{dist:.4f}  {nearest_str:^11}  {ood_str:^5}  {ok:^4}  {short}"
    )
...
# OOD 통계
ood_count = sum(1 for line in lines if "OOD" in line and line[0].strip().isdigit())
lines.append("")
lines.append("[ threshold ]")
for c, thr in enumerate(detector.class_thresholds):
    lines.append(f"  L{c}: {thr:.4f}")

with open("predict_cosine_results.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(lines))
print("done")
