"""
재난문자 긴급성 분류 — KLUE-BERT 파인튜닝

사용법:
  python train.py --data 재난문자_레이블링결과.xlsx
  python train.py --data 재난문자_레이블링결과.xlsx --epochs 5 --batch_size 32
"""
...
def main():
    ...
    # ── 데이터 ────────────────────────────────────
    print("\n데이터 로드 중...")
    train_df, val_df, test_df = load_and_split(args.data, seed=args.seed)
    ...
    log_df.to_csv('results/train_log.csv', index=False, encoding='utf-8-sig')
    save_training_curves(log_df, 'results/training_curves.png')
    ...
    print(f"  학습 로그:  results/train_log.csv")
    print(f"  평가 결과:  results/evaluation_report.txt")
    print(f"  혼동행렬:   results/confusion_matrix.png")
    print(f"  학습 곡선:  results/training_curves.png")
