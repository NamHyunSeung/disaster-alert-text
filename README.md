...
### 단계 9: v9n 기반 소량 fine-tune (v18, 진행 예정)

v9n의 masked 98.75%를 유지하면서 신종 감염병 OOD를 해결하기 위한 실험.

**전략**: init_model=v9n + 감염병 합성 100건만 추가(dedup_v7) + LR=1e-6으로 catastrophic forgetting 억제

| 파라미터 | v14 (실패) | **v18 (계획)** | 변경 이유 |
|----------|-----------|---------------|---------|
| init_model | v9n | v9n | — |
| data | dedup_v6 (감염병 500건) | **dedup_v7 (감염병 100건)** | 감염병 비중 최소화 |
| LR | 5e-6 | **1e-6** | catastrophic forgetting 억제 |
| alpha(KL) | 0.0 | **0.5** | 마스킹 일관성 유지 |
| max_epochs | 10 | **5** | 과학습 방지 |
| ES patience | 3 | **2** | 조기 종료 강화 |

**목표**: masked MacroF1 ≥ 97.5%, 신종 감염병 OOD L3 정분류
