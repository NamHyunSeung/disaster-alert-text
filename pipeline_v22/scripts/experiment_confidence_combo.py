"""
6단계: confidence(모델 신뢰도) + score(KNN 거리) 결합 검증 실험

지금까지 K/PCT 전역 파라미터 조정과 LOF 로컬 밀도 정규화 모두
'같은 거리 신호를 다른 방식으로 재가공'하는 시도였고 둘 다 실패함
(=> 거리 신호 자체에는 '정분류인데 멀리 있는 샘플'과 '오분류라서 멀리 있는 샘플'을
    가를 정보가 부족하다는 뜻)

이번에는 완전히 독립적인 신호인 모델의 softmax confidence를 결합한다:

  combo_score = raw_score * (1 - confidence)

  - confidence가 높을수록(모델이 확신) -> (1-confidence) 작아짐 -> score가 깎여 살아남음 (구제)
  - confidence가 낮을수록(모델이 헷갈림) -> (1-confidence) 커짐 -> score 거의 유지 (계속 거부)

새 threshold 파라미터를 추가하지 않고 기존 PCT=96 percentile 방식을 그대로 재사용
(score 분포가 바뀌면 percentile이 새 분포에 맞춰 threshold를 알아서 재산출하기 때문)

실행: python pipeline_v22/scripts/experiment_confidence_combo.py
"""
...
OUT_TXT    = os.path.join(ROOT, 'results', 'experiment_confidence_combo.txt')
BATCH_SIZE = 64
MAX_LEN    = 128
PCT        = 96     # 기존과 동일하게 고정 (score 분포가 바뀌어도 percentile이 알아서 재산출)
K          = 20     # 기존과 동일 (K는 결과에 거의 영향 없음이 확인됨)
SAFE_FLOOR = 4
...
def main():
    ...
    # ── 결합 score = raw_score * (1 - confidence) ───────────────────
    # confidence가 높을수록(확신) score가 깎여 살아남고, 낮을수록(헷갈림) score 유지
    train_combo_score = train_raw_score * (1.0 - train_confs)
    val_combo_score   = val_raw_score   * (1.0 - val_confs)
    msg_combo_score   = msg_raw_score   * (1.0 - msg_confs)

    print("PCT=96 기준으로 raw / combo 두 방식 채점 중...")
    raw = eval_method('raw(기존)', train_raw_score, val_raw_score, msg_raw_score,
                      train_labels, train_preds, val_labels, msg_base_preds, misclassified)
    combo = eval_method('combo(결합)', train_combo_score, val_combo_score, msg_combo_score,
                        train_labels, train_preds, val_labels, msg_base_preds, misclassified)
    ...
    lines = []
    lines.append("=" * 100)
    lines.append(f"[6단계] confidence + score 결합 검증 실험 (K={K}, PCT={PCT} 고정)")
    lines.append("=" * 100)
    lines.append("raw(기존)  : score = mean(KNN거리)")
    lines.append("combo(결합): score = mean(KNN거리) * (1 - 모델 confidence)")
    lines.append("            (확신 높으면 score 깎임->구제, 확신 낮으면 score 유지->계속 거부)")
    ...
    # 20개 메시지 상세 비교 (confidence 값도 함께 표시)
    lines.append("=" * 100)
    lines.append("[20개 메시지 OOD 판정 상세: raw vs combo (confidence 포함)]")
    lines.append("=" * 100)
    lines.append(f"{'#':>2} {'정답':^5} {'기본예측':^7} {'conf':^6}   {'raw':^5} {'combo':^5}   메시지")
    lines.append("-" * 100)
    for j in range(len(messages)):
        ...
        lines.append(f"{idx:>2}{novel_mark} L{true_label:^3}  {LABEL_NAMES[base_pred]:^6} {conf*100:>5.1f}%   {r_flag:^5} {c_flag:^5}   {short}")
    lines.append("")
    ...
    for i in sample_idx:
        ...
        lines.append(f"  - 정답=L{train_labels[i]} 예측=L{train_preds[i]}(conf={train_confs[i]*100:.1f}%) "
                     f"raw_score={train_raw_score[i]:.6f} combo_score={train_combo_score[i]:.6f} | {text}")
    lines.append("")

    # 새로 억울하게 거부된 학습 샘플 일부 예시 (부작용 확인용)
    ...
    lines.append("[해석 가이드]")
    lines.append("- '정분류였는데 거부'(방향 B)가 raw 대비 combo에서 줄었다면 -> confidence 결합으로 억울한 거부가 실제로 줄어드는 효과")
    lines.append("- '오분류라서 거부'(방향 A 포착력)가 combo에서도 유지/개선됐다면 -> confidence가 높은 '확신에 찬 오분류'를 놓치지 않고 부작용 없이 보정된 것")
    lines.append("- '노벨 7개중 탐지'가 combo에서 줄었다면 -> confidence가 높게 나온 노벨 샘플(모델이 엉뚱한 라벨에 확신을 가진 경우)까지 구제되어 탐지력이 손실되는 트레이드오프")
    lines.append("- 세 조건을 모두 만족하면 confidence 결합 방식이 raw보다 더 나은 대안이 될 수 있음")
